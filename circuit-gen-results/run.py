...
def run_amazon(s):
    res = subprocess.run(["/usr/bin/time", "-l", "venv/bin/python3", "amazon-braket/run_braket.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
    store("amazon", s, t, mem)

def run_aria(s):
    res = subprocess.run(["/usr/bin/time", "-l", "venv/bin/python", "AriaQuanta/run_aria.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
    store("aria", s, t, mem)

def run_cirq(s):
    res = subprocess.run(["/usr/bin/time", "-l", "venv/bin/python", "cirq/rin_cirq.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
    store("cirq", s, t, mem)
...
def run_ket(s):
    res = subprocess.run(["/usr/bin/time", "-l", "venv/bin/python", "ket/run_ket.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
    store("ket", s, t, mem)

def run_pennylane(s):
    res = subprocess.run(["/usr/bin/time", "-l", "venv/bin/python", "pennylane/run_penny.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
    store("pennylane", s, t, mem)

def run_projectq(s):
    res = subprocess.run(["/usr/bin/time", "-l", "venv/bin/python", "projectQ/run_projectq.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
    store("projectq", s, t, mem)

def run_pytket(s):
    res =  subprocess.run(["/usr/bin/time", "-l", "venv/bin/python", "pytket/run_pytket.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
    store("pytket", s, t, mem)

def run_qiskit(s):
    res = subprocess.run(["/usr/bin/time", "-l", "venv/bin/python", "qiskit/run_qiskit.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
    store("qiskit", s, t, mem)

def run_qrips(s):
    res = subprocess.run(["/usr/bin/time", "-l", "venv/bin/python", "qrisp/run_qrisp.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
    store("qrisp", s, t, mem)
...
def run_straw(s):
    res = subprocess.run(["/usr/bin/time", "-l", "venv/bin/python", "strawberry_fields/run_straw.py", f"{s}"], capture_output=True, text = True)
    t, mem = extract(res)
...
for s in x:
    ...
    run_pytket(s)
    print("quipper")
    run_quipper(s) # for your own sanity, dont do it
