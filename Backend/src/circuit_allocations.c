...
circuit_t *init_circuit() {
    ...
    circ->gate_index_layer_qubits = malloc(LAYER_BLOCK * sizeof(int *));
    for (int i = 0; i < LAYER_BLOCK; ++i) {
        circ->gate_index_layer_qubits[i] = malloc(QUBIT_BLOCK * sizeof(int));
        memset(circ->gate_index_layer_qubits[i], 0xFF, QUBIT_BLOCK * sizeof(int));
    }
    
    // allocate layer informations
    circ->used_indices_per_qubit = calloc(QUBIT_BLOCK, sizeof(num_t));
    circ->allocated_indices_per_qubit = malloc(LAYER_BLOCK * sizeof(num_t));
    
    circ->layer_on_qubit = malloc(QUBIT_BLOCK * sizeof(layer_t *));
    for (int i = 0; i < QUBIT_BLOCK; ++i) {
        circ->layer_on_qubit[i] = malloc(QUBIT_INDEX_BLOCK * sizeof(layer_t));
        circ->allocated_indices_per_qubit[i] = QUBIT_INDEX_BLOCK;
    }
...
void allocate_more_qubits(circuit_t *circ, gate_t *g) {
    ...
    circ->used_indices_per_qubit = realloc(circ->used_indices_per_qubit, max * sizeof(num_t));
    memset(circ->used_indices_per_qubit + circ->allocated_qubits, 0, (max - circ->allocated_qubits) * sizeof(num_t));
    
    // increase register storing which layers occupy qubits
    circ->layer_on_qubit = realloc(circ->layer_on_qubit, max * sizeof(layer_t *));
    circ->allocated_indices_per_qubit = realloc(circ->allocated_indices_per_qubit, max * sizeof(layer_t *));
    ...
    for (int lay = 0; lay < circ->allocated_layer; ++lay) {
        circ->gate_index_layer_qubits[lay] = realloc(circ->gate_index_layer_qubits[lay], max * sizeof(int));
        memset(&circ->gate_index_layer_qubits[lay][circ->allocated_qubits], 0xFF, (max - circ->allocated_qubits) * sizeof(int));
    }
...
void allocate_more_layer(circuit_t *circ, layer_t min_possible_layer) {
    ...
    if (min_possible_layer < circ->allocated_layer) return;
    ...
    for (int i = circ->allocated_layer; i < new; ++i) circ->sequence[i] = malloc(GATES_PER_LAYER_BLOCK * sizeof(gate_t));
    
    
    circ->gate_index_layer_qubits = realloc(circ->gate_index_layer_qubits, new * sizeof(int *));
    for (int i = circ->allocated_layer; i < new; ++i) {
        circ->gate_index_layer_qubits[i] = malloc(circ->allocated_qubits * sizeof(int));
        memset(circ->gate_index_layer_qubits[i], 0xFF, circ->allocated_qubits * sizeof(int));
    }
...
void allocate_more_indices_per_qubit(circuit_t *circ, int loc){
    if (circ->used_indices_per_qubit[loc] == circ->allocated_indices_per_qubit[loc]) {
        circ->layer_on_qubit[loc] = realloc(
            circ->layer_on_qubit[loc],
            (circ->allocated_indices_per_qubit[loc] + QUBIT_INDEX_BLOCK) * sizeof(layer_t)
        );
        circ->allocated_indices_per_qubit[loc] += QUBIT_INDEX_BLOCK;
    }
}
