...
typedef struct circuit_s {
    ...
    int simulate; // 0 = tracking-only (count gates, don't store), 1 = full simulation (store gates)

    size_t gate_count; // Running count of gates (incremented by run_instruction in both modes)
