...
qc_sequence_t *qc_c_cmp_cq_equal_seq(int bits, int64_t value) {
    ...
    if (bits == 1) {
        ...
        mcx_layers = mcx_decomp_layers(bits + 1);
    }
    int num_layers = num_cx_gates + mcx_layers + num_cx_gates;

    qc_sequence_t *seq = alloc_sequence(num_layers, (int)bits + 2);
    if (seq == NULL) {
        free(bin);
        return NULL;
    }
    ...
    if (bits == 1) {
        ...
        emit_mcx_decomp(seq, &current_layer, 0, controls, bits + 1, anc_start);
    }
...
/*
 * Note on controlled CQ variants (qc_c_cmp_cq_less_seq, etc.):
 * There are no public C-level wrapper functions for the controlled variants.
 * The _seq() functions above are correct and used by the Python layer
 * (quantum-cython-types), which is responsible for allocating ancilla qubits
 * and building the qubit_array mapping before calling qc_run_instruction().
 *
 * Note on qc_cmp_qq_less:
 * The QQ less-than wrapper lives in arithmetic_dispatch.c and correctly
 * allocates its own temp register and borrow ancilla. No fix needed.
 */
